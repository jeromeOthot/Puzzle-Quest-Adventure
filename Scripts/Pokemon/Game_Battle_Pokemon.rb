...
NB_ENERGIES = 8
  ...
  ############################################################################
  # Fait subir des d�gats � l'enemie
  ############################################################################
  def attackEnemy()
        isAttackWork = true
...
#~      @gridBoard[[5,4]] = gem
  ...
  #############################################################################
  # Cr�e les index des gem pour le board
  #############################################################################
  ...
  #############################################################################
  def putAnotherRandomGem(x,y)
    if( @gridGemsIndex[[x-1,y]] != nil && @gridGemsIndex[[x-1,y]] == GEM_INDEX_EXP )
  ...
  def getRandomGemId
    ...
    return rand(NB_ENERGIES)+1.to_i
  #  case firstSelection
      #Gem �lemental
    #  when 0 .. 50
  #      return getRandomElementalGemId
...
#    end
  ...
  #############################################################################
   ...
   def checkIs3sameIndex(x, y)
      ...
      if( check3SameIndexHorizontal(x,y) || check3SameIndexvertical(x, y) )
        index = 0
      end
  ...
  #############################################################################
  #V�rifie si 2 gem Multi sont cote � cote
  #############################################################################
  def check2MultiGemBeside(x, y)
      if(@gridGemsIndex[[x, y]] == @gridGemsIndex[[x-1, y]])
        return true
      elsif(@gridGemsIndex[[x, y]] == @gridGemsIndex[[x, y-1]])
        return true
      else
        return false
      end
  ...
  #############################################################################
  #V�rifie les combine horizontal des gem Multi avec gem �l�mental
  #############################################################################
  def checkCombineMultiIndexHorizontal(x, y)

#~       indexLeft1 = @gridGemsIndex[[x-1, y]
#~       indexLeft2 = @gridGemsIndex[[x-2, y]
...
#~       end
  ...
  #############################################################################
  #V�rifie les combine vertical de gem Multi avec gem �l�mental
  #############################################################################
  ...
  #############################################################################
  #V�rifie si le gem est entour� de au moin x gem des 4 c�t�
  #############################################################################
