...
class Game_Party < Game_Unit
  ...
  def initialize
    ...
    @target_actor_id = 0
    @actors = []
    @effect_factory = Effects_Factory.new
    init_all_items
  end
  #--------------------------------------------------------------------------
  ...
  def item_container(item_class)
    return @consumableItems   if item_class == RPG::Item
...
#~         setItemToMagic(item)
#~       #Rare
#~       when 51..65; item.set_rarety( 3 ) 
#~         
#~       #Unique  
#~       when 66..80;  item.set_rarety( 4 )
#~         
#~       #Set  
#~       when 81..100; item.set_rarety( 5 ) 
#~          
#~     end
  end
  
  def setItemToMagic(item)
      item.set_rarety( 2 )
      preffix = @effect_factory.create_effect(1, 5, false)
      surffix = @effect_factory.create_effect(1, 5, true) 

      # item.effects.push(preffix)
      # item.effects.push(surffix)
      item.name = preffix.name + item.name + surffix.name
      item.description = preffix.description + "\n" + surffix.description
  end
end
