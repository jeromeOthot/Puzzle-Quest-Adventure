...
# Type            Key         Display name      
  ...
  Item     =  [PearlKey::N1,      'n1']            # Item  usage
  Item2    =  [PearlKey::N2,      'n2']            # Item2 usage
  Item3    =  [PearlKey::N3,      'n3']            # Item3  usage
  Item4    =  [PearlKey::N4,      'n4']            # Item4 usage
...
class Game_Player < Game_Character
  ...
  def update_tool_usage
    ...
    unless actor.assigned_item3.nil?
      ...
      use_item(actor.assigned_item3.id) if trigger_tool?(4, :mouse) 
    end
    unless actor.assigned_item4.nil?
      use_item(actor.assigned_item4.id) if trigger_tool?(Key::Item4[0], :keys)
      use_item(actor.assigned_item4.id) if trigger_tool?(4, :mouse) 
    end
  ...
  def update_menu_buttons
    ...
    if !PearlKernel::SinglePlayer and PearlKey.trigger?(Key::WorldMap)
      @pearl_menu_call = [:combat, 2]
    end
  end
...
class Game_Party < Game_Unit
  ...
  def set_item(actor_id, item_id, slot)
    ...
    when Key::Item3[1].to_sym  then actor.assigned_item3 = item
    when Key::Item4[1].to_sym then actor.assigned_item4  = item
    end
  end
end
...
class Game_Follower < Game_Character
  ...
  def update
    ...
    @transparent = lying_down?
    
  end
  
  def lying_down?
    return true if visible? && !battler.deadposing.nil?  && 
...
class Game_Actor < Game_Battler
  ...
  attr_accessor :assigned_skill3, :assigned_skill4, :assigned_item3, :assigned_item4
  alias falcaopearl_cooldown_setup setup
  def setup(actor_id)
    @usability = [nil, nil, nil, nil, nil, nil, nil, nil, nil, nil]
    @primary_use = 1
  ...
  def apply_usability
    ...
    @usability[4] = usable?(@assigned_item3) if !@assigned_item3.nil?
    @usability[5] = usable?(@assigned_item4) if !@assigned_item4.nil?
    
    @usability[6] = usable?(@assigned_skill) if !@assigned_skill.nil?
    @usability[7] = usable?(@assigned_skill2) if !@assigned_skill2.nil?
    @usability[8] = usable?(@assigned_skill3) if !@assigned_skill3.nil?
    @usability[9] = usable?(@assigned_skill4) if !@assigned_skill4.nil?
  end
...
class Game_Enemy < Game_Battler
  ...
  attr_accessor :kill_skill, :body_sized, :esensor, :boss_hud
