...
class Window_Primaryuse < Window_Command
  ...
  def make_command_list
    ...
    add_command('Item '  + Key::Item3[1],    'Item ' + Key::Item3[1])
    add_command('Item '  + Key::Item4[1],   'Item '  + Key::Item4[1])
...
class Scene_CharacterSet < Scene_MenuBase
  def start
    ...
    @primary_use.set_handler('Item '   + Key::Item3[1],    method(:apply_item))
    @primary_use.set_handler('Item '   + Key::Item4[1],   method(:apply_item))
  ...
  def apply_item
    ...
    when 'Item '   + Key::Item3[1]   then actor.primary_use = 5
    when 'Item '   + Key::Item4[1]  then actor.primary_use = 6
      
    when 'Skill '  + Key::Skill[1]  then actor.primary_use = 7
    when 'Skill '  + Key::Skill2[1] then actor.primary_use = 8
    when 'Skill '  + Key::Skill3[1] then actor.primary_use = 9
    when 'Skill '  + Key::Skill4[1] then actor.primary_use = 10
    end
...
class Window_ItemSelect < Window_HorzCommand
  ...
  def make_command_list
    add_command("Armes",  :weapon)
    add_command("Boucliers",   :shield)
    add_command("Armures",  :armor)
    add_command("Casques",  :helmet)
    add_command("Bottes",   :boot)
...
class Window_SlotConfirm < Window_Command
  ...
  def make_command_list
    ...
    when :item
      ...
      add_command('Slot ' + Key::Item3[1],   :slot3)
      add_command('Slot ' + Key::Item4[1],   :slot4)
    when :skill
...
class Window_ActorQuickTool < Window_Selectable
  def initialize(x=0, y=124, w=460, h=148, split = false)
    super(x, y,  (split == true ? w/2 : w), h)
    @split = split
    unselect
  end
  
  def item()        return @data[self.index] end
  def col_max()     return @split ? 4 : 8    end 
  def spacing()     return 1                 end  
  
  #--------------------------------------------------------------------------
  # * Create Item List
  #--------------------------------------------------------------------------
  def make_item_list(kind)
    @data = $game_party.all_items.select {|item| include?(item, kind) }
    $game_party.actors.each { |id| 
      $game_actors[id].equips.each { |item| 
        @data.push(item) if include?(item, kind) 
      }
    }
    @data.sort!
    @data.push(nil) if include?(nil, kind) 
 ...
 def refresh(actor, kind)
    ...
    @item_max = @data.size
    if @item_max > 0
      self.contents = Bitmap.new(width, row_max * 24)
      for i in 0...@item_max
        draw_item(i)
      end
    end
...
#Window qui contient le contenu du coffre actuel
class Window_ChestContain < Window_Selectable
  def initialize(x=0, y=124, w=230, h=148)
    super(x, y, w, h)
    unselect
  end
  ...
  def refresh(chestIndex)
    self.contents.clear if self.contents != nil
    contents.draw_text(0, 0, 212, 32, "Coffre")
    @item_max = $game_variables[chestIndex].count
    if @item_max > 0
      self.contents = Bitmap.new(width, row_max * 24)
      for i in 0...@item_max
  ...
  def draw_item(index)
    item = $data_items.at($game_variables[$chestIndex].at(index))
...
class Scene_QuickTool < Scene_MenuBase
  ...
  def start()
    super
    x = Graphics.width / 2 - 460 / 2 
    y = 0
    #@top_text = Window_Base.new(x, y - 156, 460, 85)
    @statust = ['Ready', 0]
    #refresh_top_info
    @type_select = Window_ItemSelect.new(x, y)
    @type_select.set_handler(:weapon,     method(:refresh_tools))
    @type_select.set_handler(:shield,      method(:refresh_tools))
    @type_select.set_handler(:armor,       method(:refresh_tools))
    @type_select.set_handler(:helmet,      method(:refresh_tools))
    @type_select.set_handler(:boot,       method(:refresh_tools))
    @type_select.set_handler(:item,      method(:refresh_tools))
    @type_select.set_handler(:cancel,     method(:refresh_cancel))
    @type_select.set_handler(:down,     method(:refresh_tools))
    @type_index = @type_select.index
    @items_w = Window_ActorQuickTool.new( @type_select.x, @type_select.y+50, 460, 148, $split)
    @items_w.refresh($game_player.actor, @type_select.current_symbol)
    
    #Si on est dans un coffre
    if($split == true)
      @chestContain = Window_ChestContain.new( @type_select.x+230, @type_select.y+50, 230, 148)
      @chestContain.refresh($chestIndex)
    end
  ...
  def refresh_description
    ...
    @description.draw_text_ex2(0, -4, @items_w.item.name + ": " + @items_w.item.description)
  end
  ...
  def perform_item_ok
    ...
    when :armor
      ...
      equip_play
    when :boot
      actor.change_equip_by_id(2, @items_w.item.id)
      equip_play
    when :helmet
      actor.change_equip_by_id(3, @items_w.item.id)
      equip_play
    when :shield
      actor.change_equip_by_id(4, @items_w.item.id)
      equip_play
    when :item
      activate_slots
    #when :skill
