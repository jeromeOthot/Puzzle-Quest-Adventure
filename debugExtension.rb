...
class Window_Debug < Window_Command
...
class Window_DebugSwitch < Window_Debug
...
class Window_DebugVariable < Window_Debug
...
class Window_DebugBattle < Window_Debug
...
class Window_DebugCommonEvent < Window_Debug
...
class Window_DebugItem < Window_Debug
...
class Scene_Debug < Scene_MenuBase
  ...
  def create_command_window
    ...
    @command_window.set_handler(:capacities,  method(:command_capacities))
    @command_window.set_handler(:titles,      method(:command_titles))
    ...
    @command_window.set_handler(:actors,      method(:command_actors))
  ...
  def refresh_help_window
    if @command_window.active
      ...
      when :switches
        ...
        text += "Press A to toggle switch.\n"
        text += "Yellow switches are on and named.\n"
        text += "Red switches are on but are unnamed."
      when :variables
        text  = "Adjust variables.\n"
        text += "Press A to modify variable.\n"
        text += "Yellow aren't 0 and named.\n"
        text += "Red aren't 0 and are unnamed."
      when :battle
        text  = "Test Battles.\n"
        text += "Select a battle and press A.\n"
        text += "These battles are all found\n"
        text += "within the database."
      when :events
        text  = "Test Common Events.\n"
        text += "Select an event and press A.\n"
        text += "You will automatically be taken\n"
        text += "to previous scene to run event."
      when :skills
        text  = "Learn Skills.\n"
        text += "Press A to learn a skill\n"
      when :capacities
        text  = "Learn Capacities.\n"
        text += "Press A to learn a capacity\n"
      when :titles
        text  = "Earn Titles.\n"
        text += "Press A to earn a title\n"
      when :recipes
        text  = "Learn Recipes.\n"
        text += "Press A to learn a recipe\n"
      when :cooking
        text  = "Adjust Recipe Mastery.\n"
        text += "Press Right to use recipe.\n"
        text += "Hold SHIFT for increments of 10.\n"
        text += "Hold CTRL for increments of 100."
      when :party
        text  = "Modify Party Parameters.\n"
        text += "Press A to modify this parameter\n"
        text += "Hold SHIFT for increments of 10.\n"
        text += "Hold CTRL for increments of 100."
      when :items
        text  = "Adjust items.\n"
    ...
    if @skill_window.active || 
       @capacity_window.active || 
       @title_window.active || 
       @cooking_window.active || 
       @actor_window.active
      @help_window.draw_actor_face(@actor, 0, 12)
      @help_window.draw_text_ex(84, 0, text)
    else
